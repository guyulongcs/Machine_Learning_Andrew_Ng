...
for i=1:m
    yo=y3(i,:);
    for j=1:num_labels
        res=res+(-yo(j)*log(hypo(i,j))-(1-yo(j))*log(1-hypo(i,j)));
    end
end
res=res/m;
J=res;
...
delta3=a3-y3;
delta2Tmp=delta3*Theta2;
delta2Tmp=delta2Tmp(:,2:end);
delta2=delta2Tmp.*sigmoidGradient(z2);
...
for t=1:m
    BigDelta2=BigDelta2+delta3(t,:)'*a2(t,:);
    BigDelta1=BigDelta1+delta2(t,:)'*a1(t,:);
end
